...
public class Packager {
    ...
    static void HandleExampleBundle() {
        ...
        //改写后的ab资源打包理论上只有多资源种类时需要在此添加，填写顶层文件夹名和后缀会自动遍历子目录添加
        AutoAddObj("*.prefab", "GameObj");
        AutoAddObj("*.prefab", "UI");
    ...
    private static void AutoAddObj(string type,string foldName)
    {
        string path = Application.dataPath + AppConst.PrefabPath+foldName;
        UnityEngine.Debug.LogError("prefab path:" + path);
        if(!Directory.Exists(path))
        {
            UnityEngine.Debug.LogError("打包预制体文件夹不存在！");
            return;
        }
        DirectoryInfo dir = new DirectoryInfo(path);
        foreach(var fold in dir.GetDirectories())
        {
            if(fold.GetDirectories().Length>0) //递归遍历
            {
                AutoAddObj(type, foldName + "/" + fold.GetDirectories()[0].Name);
            }

            UnityEngine.Debug.Log("bundle foldName:" + fold.Name);
            AddBuildMap(fold.Name + AppConst.ExtName, type, "Assets" + AppConst.PrefabPath + foldName + "/" + fold.Name);
        }
    }
